...
function custom_orders_list_column_content_shutter($column, $post_id)
{
  ...
  if ($column == 'order-shipping') {
    if ($myOrder['delivery'] === 'air') {
      echo '<strong>By Air</strong>';
    }
  }
  ...
  if ($column == 'order-price-dolar') {
    echo '$' . number_format($myOrder['usd_price'], 2);
  }

  if ($column == 'order_reff') {
    // Get custom post meta data
    $my_var_one = get_post_meta($post_id, 'cart_name', true);
    if (!empty($my_var_one))
      echo $my_var_one;
  ...
  if ($column == 'order_QB') {
    ...
    if ($QB_invoice == true) {
      ...
      if ($billing_company == 'Lifetime Shutters') {
        echo 'Invoice Created';
      } else {
        echo '<button class="send_qcuickbooks_invoice btn btn-primary" id-order="' . $post_id . '" >Create QB invoice</button>';
      }
    }
  }

  if ($column == 'container-id') {
    // Get custom post meta data
  ...
  if ($column == 'order_status') {
    ...
    if ($order_status == 'revised' || $order_status == 'inrevision' || $order_status == 'on-hold') {
      echo 'color: yellow;';
    }
    // }
