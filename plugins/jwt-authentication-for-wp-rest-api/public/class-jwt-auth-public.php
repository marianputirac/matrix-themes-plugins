...
class Jwt_Auth_Public
{
    ...
    public function generate_token($request)
    {
        ...
        $user_app_priv = '';
/*th*/  $user_text_alert = 'Your subscription is disabled. Please call Mike or email Tudor at order@lifetiemshutters.com to grant you further access';


        $user_app_priv = get_user_meta($user->data->ID,'app_access',true);

        if( $user_app_priv == 'yes' ){
        ...
        $data = array(
            ...
            'app_access' => $user_app_priv,
/*th*/      'text_alert' => $user_text_alert,
        );
    ...
    public function validate_token($output = true)
    {
        ...
        try {
            ...
            /** So far so good, validate the user id in the token and see if user have app_access */
            $user_app_priv = get_user_meta($user->data->ID,'app_access',true);

            if ( isset($token->data->user->id) ) {
                $id_user = $token->data->user->id;
                $user_app_priv = get_user_meta($id_user,'app_access',true);

                if( $user_app_priv == 'no' ){
                    return new WP_Error(
                        'jwt_auth_bad_request',
                        'User ID with no app access',
                        array(
                            'status' => 403,
                        )
                    );
