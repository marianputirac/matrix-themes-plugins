...
function stg_sc_ticket_form($atts,$content,$tag)
{
    ...
    if(!empty($content) && $tag == "ticket-form")
    {
        ...
        if($form)
        Stg_Helper_Template::getTemplate('stg-ticket-form-by-id', array("formId" => $form->term_id, "atts" => $atts));

    }else{
        $defaultForm= \StgHelpdesk\Helpers\Stg_Helper_Custom_Forms::getDefaultForm();

        if($defaultForm)
            Stg_Helper_Template::getTemplate('stg-ticket-form-by-id', array("formId" => $defaultForm->term_id, "atts" => $atts));
        else
