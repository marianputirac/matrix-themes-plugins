...
class WC_Admin_List_Table_Orders extends WC_Admin_List_Table {
	...
	protected function render_order_number_column() {
		...
		} elseif ( $this->object->get_billing_first_name() || $this->object->get_billing_last_name() ) {
		...
		if ( $this->object->get_status() === 'trash' ) {
			echo '<strong>LF0' . esc_attr( $this->object->get_order_number() ) . ' ' . esc_html( $buyer ) . '</strong>';
		} else {
			...
			echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $this->object->get_id() ) ) . '&action=edit' ) . '" class="order-view"><strong>LF0' . esc_attr( $this->object->get_order_number() ) . ' ' . esc_html( $buyer ) . '</strong></a>';
		}
	}
				...
				<div class="wc-backbone-modal-content">
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							...
							<h1><?php echo esc_html( sprintf( __( 'Order LF0%s', 'woocommerce' ), '{{ data.order_number }}' ) ); ?></h1>
	...
	protected function render_filters() {
		...
		if ( ! empty( $_GET['_customer_user'] ) ) { // phpcs:disable  WordPress.Security.NonceVerification.NoNonceVerification
			...
			$user_string = sprintf(
				...
				esc_html__( '%1$s (LF0%2$s &ndash; %3$s)', 'woocommerce' ),
				$user->display_name,
				absint( $user->ID ),
				$user->user_email
			);
