                ...
                foreach ($matched_shipping_methods as $shipping_method_id) {
                    ...
                    if (!empty($cart_array)) {
                        foreach ($cart_array as $woo_cart_item_key => $value) {
                            $cart_based_qty += floatval($value['quantity']);
                            ...
                            if ($property_total < 1 && $property_category != 'Batten') {
                                $cart_based_sqm += 1 * floatval($value['quantity']);
                            } else {
                                $cart_based_sqm += floatval($property_total) * floatval($value['quantity']);
                            }
                        }
                    }
                    ...
                    $getFeesPerSqmFlag = get_post_meta($shipping_method_id, 'sm_fee_chk_sqm_price', true);
                    ...
                    if ($getFeesPerQtyFlag == 'on') {
                        ...
                        } elseif ($getFeesPerQty == 'qty_product_based') {
                            $cost = $costs + (($products_based_qty - 1) * $extraProductCost);
                        }
                        /*Per Qty Condition end*/
                    } /*Per Qty Condition start*/
                    elseif ($getFeesPerSqmFlag == 'on') {
                        
                        $cost = number_format($costs, 2) * number_format($cart_based_sqm, 2);
